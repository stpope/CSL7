...
Sample CGestalt::framePeriod() { return mFramePeriod; }
...
unsigned CGestalt::MIDIOutDevice() { return mMIDIOutDevice; };
void CGestalt::setMIDIOutDevice(unsigned dev) { mMIDIOutDevice = dev; };
...
void Model::detachObserver (Observer * o) {
	unsigned count = (unsigned) mObservers.size();
	int i;
	for (i = 0; i < count; i++)
		if (mObservers[i] == o)
			break;
