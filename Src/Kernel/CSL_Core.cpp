...
void Buffer::checkBuffers() noexcept(false) {
...
void Buffer::allocateBuffers() noexcept(false) {
...
void Buffer::copyHeaderFrom(Buffer & source) noexcept(false) {
...
void Buffer::copySamplesFrom(Buffer & source) noexcept(false) {
	if ((source.mNumChannels > mNumChannels) || (source.mNumFrames > mNumAlloc)) {
...
void Buffer::copyOnlySamplesFrom(Buffer & source) noexcept(false) {
	if ( /* (source.mNumChannels > mNumChannels) || */ (source.mNumFrames > mNumAlloc)) {
...
void Buffer::copySamplesFromTo(Buffer & source, unsigned offset) noexcept(false) {
...
bool UnitGenerator::checkFanOut(Buffer & outputBuffer) noexcept(false) {
...
void UnitGenerator::handleFanOut(Buffer & outputBuffer) noexcept(false) {
...
void UnitGenerator::nextBuffer(Buffer & outputBuffer) noexcept(false) {
...
void UnitGenerator::nextBuffer(Buffer & outputBuffer, unsigned outBufNum) noexcept(false) {
	for (unsigned i = 0; i < mNumChannels; i++)
...
void Controllable::pullInput(Port * thePort, unsigned numFrames) noexcept(false) {
#ifdef CSL_DEBUG
...
void Controllable::pullInput(Port * thePort, Buffer & theBuffer) noexcept(false) {
	if (thePort == NULL)
		return;
	UnitGenerator * theUG = thePort->mUGen;			// get its UGen
	if (theUG == NULL)								// if it's a static variable
...
void Effect::pullInput(Buffer & outputBuffer) noexcept(false) {
#ifdef CSL_DEBUG
...
void Effect::pullInput(unsigned numFrames) noexcept(false) {
#ifdef CSL_DEBUG
	logMsg("Effect::pullInput");
#endif
	Port * iPort = mInputs[CSL_INPUT];
...
void FanOut::nextBuffer(Buffer & outputBuffer,  unsigned outBufNum) noexcept(false) {
	throw LogicError("Asking for mono nextBuffer of a FanOut");
}

// nextBuffer just pulls the input every "mOutputs" calls

void FanOut::nextBuffer(Buffer & outputBuffer) noexcept(false) {
...
void Splitter::nextBuffer(Buffer & outputBuffer,  unsigned outBufNum) noexcept(false) {
...
void Splitter::nextBuffer(Buffer & outputBuffer) noexcept(false) {
...
void Joiner::nextBuffer(Buffer & outputBuffer,  unsigned outBufNum) noexcept(false) {
	throw LogicError("Asking for mono nextBuffer of a Joiner");
}

// nextBuffer calls each of the input channels as a mono channel

void Joiner::nextBuffer(Buffer & outputBuffer) noexcept(false) {
	if (outputBuffer.mNumChannels != mNumChannels) {
...
void Writeable::writeBuffer(Buffer & inputBuffer, unsigned bufNum) noexcept(false) {
				// no-op here
}

void Writeable::writeBuffer(Buffer & inputBuffer) noexcept(false) {
...
void Seekable::reset() noexcept(false) {		// reset-to-zero
...
void Interleaver::interleave(Buffer & output, SampleBuffer samples, 
			unsigned numFrames, unsigned numChannels) noexcept(false) {
...
void Interleaver::interleave(Buffer & output, short * samples, unsigned numFrames, 
			unsigned numChannels) noexcept(false) {
...
void Interleaver::interleaveAndRemap(Buffer & output, SampleBuffer samples, unsigned numFrames, 
			unsigned numChannels, unsigned *channelMap) noexcept(false) {
...
IO::IO(unsigned s_rate, unsigned b_size, int in_device, int out_device, 
	...
	logMsg("\nCreate IO: %d samps/blk @ %d Hz, %d in %d out", b_size, s_rate, in_chans, out_chans);
}
...
void IO::pullInput(Buffer & outBuffer, SampleBuffer out) noexcept(false) {
...
Buffer & IO::getInput() noexcept(false) {
	return getInput(mInputBuffer.mNumFrames, mInputBuffer.mNumChannels);
}

// really get the desired format of input

Buffer & IO::getInput(unsigned numFrames, unsigned numChannels) noexcept(false) {
	if (mNumInChannels == 0)
