///
/// CSL_Core.h -- the specification file for the core classes of CSL version 7
///
...
public:									/// Constructor: default is mono and default-size
	...
	void checkBuffers() noexcept(false);			///< allocate if not already there
	void allocateBuffers() noexcept(false);		///< fcn to malloc storage buffers
	...
	void copyFrom(Buffer & src) noexcept(false);
	void copyHeaderFrom(Buffer & source) noexcept(false);	///< copy the "header" fields of a buffer
	void copySamplesFrom(Buffer & src) noexcept(false);	///< import data from the given buffer
	void copySamplesFromTo(Buffer & src, unsigned offset) noexcept(false);	///< same with write offset
	void copyOnlySamplesFrom(Buffer & src) noexcept(false);	///< import data from the given buffer
...
//	void setName(string ch) { mName = ch; }
	...
	bool checkFanOut(Buffer & outputBuffer) noexcept(false);
	void handleFanOut(Buffer & outputBuffer) noexcept(false);
...
protected:
	...
	void pullInput(Port * thePort, Buffer & theBuffer) noexcept(false);
...
protected:
	...
	void pullInput(Buffer & outputBuffer) noexcept(false);
	void pullInput(unsigned numFrames) noexcept(false);
	virtual void trigger();						///< trigger passed on here
												/// get the input port
...
public:								/// write to the receiver
	virtual void writeBuffer(Buffer& inputBuffer) noexcept(false);
	virtual ~Writeable() { /* ?? */ };

protected:							/// write to the receiver
	virtual void writeBuffer(Buffer & inputBuffer, unsigned bufNum) noexcept(false);
};
...
public:
	...
	virtual unsigned seekTo(int position, SeekPosition whence) noexcept(false) = 0;
	virtual void reset() noexcept(false);		///< reset-to-zero
...
public:
	...
	void interleaveAndRemap(Buffer & output, SampleBuffer samples, unsigned numFrames, unsigned numChannels,
						unsigned *channelMap) noexcept(false);
...
public:
	...
	virtual void test() noexcept(false) { };	///< test the IO's graph
	...
	void pullInput(Buffer & outBuffer, SampleBuffer out = 0)  noexcept(false);

	virtual Buffer & getInput() noexcept(false);	///< Get the current input from the sound card
	virtual Buffer & getInput(unsigned numFrames, unsigned numChannels) noexcept(false);
