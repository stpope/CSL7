...
CSLComponent::CSLComponent ()
{
    ...
    playButton.reset (new juce::TextButton ("playNote"));
    addAndMakeVisible (playButton.get());
    playButton->setButtonText (TRANS("Play/Stop"));
    playButton->addListener (this);
    playButton->setColour (juce::TextButton::buttonColourId, juce::Colours::cornflowerblue);
    ...
    label.reset (new juce::Label ("CSL test",
                                  TRANS("CSL 7.0 Demos - select from the menus at the lower-left")));
    addAndMakeVisible (label.get());
    label->setFont (juce::Font (juce::Font::getDefaultSerifFontName(), 24.00f, juce::Font::plain).withTypefaceStyle ("Bold"));
    ...
    prefsButton.reset (new juce::TextButton ("new button"));
    ...
    cpuLabel.reset (new juce::Label ("new label",
                                     TRANS("0.0%")));
    addAndMakeVisible (cpuLabel.get());
    cpuLabel->setFont (juce::Font (juce::Font::getDefaultSansSerifFontName(), 15.00f, juce::Font::plain).withTypefaceStyle ("Bold"));
    cpuLabel->setJustificationType (juce::Justification::centredRight);
    cpuLabel->setEditable (false, false, false);
    cpuLabel->setColour (juce::TextEditor::textColourId, juce::Colours::black);
    cpuLabel->setColour (juce::TextEditor::backgroundColourId, juce::Colour (0x00000000));
    ...
    scaleSlider.reset (new juce::Slider ("new slider"));
    ...
    amplitudeSlider.reset (new juce::Slider ("new slider"));
    ...
    amplitudeSlider->setTextBoxStyle (juce::Slider::NoTextBox, true, 8, 20);
    amplitudeSlider->addListener (this);

    loopButton.reset (new juce::ToggleButton ("new toggle button"));
    addAndMakeVisible (loopButton.get());
    loopButton->setButtonText (TRANS("Loop"));
    loopButton->addListener (this);

    familyCombo.reset (new juce::ComboBox ("test family"));
    ...
    recordButton.reset (new juce::ToggleButton ("new toggle button"));
    ...
    label3->setBounds (250, 664, 56, 24);

    MIDIInButton.reset (new juce::TextButton ("playNote"));
    addAndMakeVisible (MIDIInButton.get());
    MIDIInButton->setButtonText (TRANS("MIDI In Device"));
    MIDIInButton->addListener (this);
    MIDIInButton->setColour (juce::TextButton::buttonColourId, juce::Colours::brown);
...
////////////////////////////// Here we go! //////////////////////////////////////////
	...
	if (error.isNotEmpty())
		juce::AlertWindow("CSL Demo",
						  "Couldn't open an output device!\n\n" + error,
						  juce::AlertWindow::WarningIcon, 0);

#ifdef READ_IO_PROPS						// overwrite the system frame rate and block size from the system
	...
	CGestalt::setBlockSize(bufSize);
	CGestalt::setFrameRate(sRate);

	theIO = new IO_CLASS(sRate, bufSize, -1, -1,			// still use the default # IO channels
...
void CSLComponent::resized()
{
    ...
    playButton->setBounds (698, getHeight() - 42, 152, 38);
    quitButton->setBounds (getWidth() - 8 - 176, 8, 176, 32);
    testCombo->setBounds (300, getHeight() - 36, 220, 24);
    oscilloscopeL->setBounds (44, 48, getWidth() - 52, proportionOfHeight (0.4200f));
    label->setBounds (447 - (558 / 2), 10, 558, 28);
    cpuLabel->setBounds (getWidth() - 71, getHeight() - 35, 56, 24);
    oscilloscopeR->setBounds (44, proportionOfHeight (0.5000f), getWidth() - 52, proportionOfHeight (0.4200f));
    VUMeterL->setBounds (25, 48, 15, juce::roundToInt (proportionOfHeight (0.4200f) * 1.0000f));
    VUMeterR->setBounds (24, proportionOfHeight (0.5000f), 15, juce::roundToInt (proportionOfHeight (0.4200f) * 1.0000f));
    scaleSlider->setBounds (32, getHeight() - 60, getWidth() - 36, 24);
    amplitudeSlider->setBounds (2, 42, 20, proportionOfHeight (0.8700f));
    loopButton->setBounds (698 + 152 - -16, getHeight() - 12 - 24, 64, 24);
    familyCombo->setBounds (115, getHeight() - 36, 122, 24);
    recordButton->setBounds ((getWidth() - 8 - 176) + -12 - 72, 37 - 24, 72, 24);
    MIDIInButton->setBounds (550, getHeight() - 40, 119, 34);
...
// JUCE 7 way
		if ( ! audioSetupComp)
			audioSetupComp = new AudioDeviceSelectorComponent
										(mAudioDeviceManager,
										0, 16, 0, 16,
										true, true, true, false);
		audioSetupComp->setSize (400, 600);
...
//		}
    ...
    else if (buttonThatWasClicked == MIDIInButton.get())
    {
        //[UserButtonCode_MIDIInButton] -- add your button handler code here..
							// Let the user select the MIDI input to use
		juce::PopupMenu menu;
		juce::Array<juce::MidiDeviceInfo> inDevices = juce::MidiInput::getAvailableDevices();
		for (unsigned i = 0; i < inDevices.size(); i++)
			menu.addItem(i + 1, (String) inDevices[i].name);

		menu.showMenuAsync(juce::PopupMenu::Options(),
			[] (int res) {
				if (res == 0) {		// user dismissed the menu without picking anything
					return;
				} else {				// user picked something
					CGestalt::setMIDIInDevice(res - 1);
				}
		});

        //[/UserButtonCode_MIDIInButton]
    }
...
void CSLComponent::audioDeviceIOCallbackWithContext (const float *const * inputChannelData, int numInputChannels,
										float * const * outputChannelData, int numOutputChannels,
										int numSamples, const juce::AudioIODeviceCallbackContext & context) {
								// put silence in the output buffers
//	logMsg("CSLComponent::audioDeviceIOCallback");
	...
	if (theIO->mGraph) {
		outBuffer.setSize(numOutputChannels, numSamples);
		...
		oscilloscopeL->audioDeviceIOCallbackWithContext (inputChannelData, numOutputChannels,
								outputChannelData, numOutputChannels, numSamples, context);
		oscilloscopeR->audioDeviceIOCallbackWithContext (inputChannelData, numOutputChannels,
								outputChannelData, numOutputChannels, numSamples, context);
		VUMeterL->audioDeviceIOCallbackWithContext (inputChannelData, 0,
								outputChannelData, numOutputChannels, numSamples, context);
		VUMeterR->audioDeviceIOCallbackWithContext (inputChannelData, 0,
								outputChannelData, numOutputChannels, numSamples, context);
...
//								outputChannelData, totalNumOutputChannels, numSamples);
		...
		if (recrding) {			// cache to (stereo) record buffer
			...
			Sample * sPtr = gFileBuffer->buffer(0) + gSampIndex;
...
<JUCER_COMPONENT documentType="Component" className="CSLComponent" componentName=""
  ...
  <LABEL name="CSL test" id="a9876f115ab3c22e" memberName="label" virtualName=""
         explicitFocusOrder="0" pos="447c 10 558 28" textCol="fffffc00"
         edTextCol="ff000000" edBkgCol="0" labelText="CSL 7.0 Demos - select from the menus at the lower-left"
  ...
  <LABEL name="new label" id="87d3c5b55ea75f76" memberName="cpuLabel"
         virtualName="" explicitFocusOrder="0" pos="71R 35R 56 24" edTextCol="ff000000"
  ...
  <SLIDER name="new slider" id="c62ea84a7afde2e3" memberName="scaleSlider"
          virtualName="" explicitFocusOrder="0" pos="32 60R 36M 24" min="0.0"
  ...
  <SLIDER name="new slider" id="78c6bd39da739ba6" memberName="amplitudeSlider"
          virtualName="" explicitFocusOrder="0" pos="2 42 20 87%" min="-5.0"
  ...
  <TOGGLEBUTTON name="new toggle button" id="2322f603f1796f48" memberName="loopButton"
                virtualName="" explicitFocusOrder="0" pos="-16R 12Rr 64 24" posRelativeX="ed1811e776eea99b"
  ...
  <COMBOBOX name="test family" id="55af3a98f2faf643" memberName="familyCombo"
            virtualName="" explicitFocusOrder="0" pos="115 36R 122 24" editable="0"
  ...
  <LABEL name="new label" id="23b1e228fb0b5e55" memberName="label2" virtualName=""
         explicitFocusOrder="0" pos="46 664 72 24" edTextCol="ff000000"
  ...
  <LABEL name="new label" id="5a2d4a92827c7701" memberName="label3" virtualName=""
         explicitFocusOrder="0" pos="250 664 56 24" edTextCol="ff000000"
         edBkgCol="0" labelText="Demo" editableSingleClick="0" editableDoubleClick="0"
         focusDiscardsChanges="0" fontname="Default font" fontsize="15.0"
         kerning="0.0" bold="0" italic="0" justification="33"/>
  <TEXTBUTTON name="playNote" id="b0871074099e9b23" memberName="MIDIInButton"
              virtualName="" explicitFocusOrder="0" pos="550 40R 119 34" bgColOff="ffa52a2a"
              buttonText="MIDI In Device" connectedEdges="0" needsCallback="1"
              radioGroupId="0"/>
</JUCER_COMPONENT>
