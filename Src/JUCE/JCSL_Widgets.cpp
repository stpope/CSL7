...
void AudioWaveformDisplay::addSample (const float samp) {
	currentOutputLevel += samp;					// rather than fabs as in the other scope version
...
void AudioWaveformDisplay::audioDeviceIOCallbackWithContext (const float * const * inData, int numIns,
								float * const * outData, int numOuts,
								int numSamples, const juce::AudioIODeviceCallbackContext & context) {
	if ( ! samplesToAverage) {			// if no interpolation, just hold onto the pointer
		circularBuffer = outData[0];
		...
		for (int i = 0; i < numOuts; ++i) {
			if (outData[i] != 0) {
				for (int j = 0; j < numSamples; ++j)
					addSample (outData[i][j]);
				break;
			}
		}
	} else {				// single channel
		for (int j = 0; j < numSamples; ++j)
			addSample (outData[whichChannel][j]);
	}
}
