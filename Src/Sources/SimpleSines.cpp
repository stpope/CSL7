...
void SimpleSine::nextBuffer(Buffer & outputBuffer, unsigned outBufNum) noexcept(false) {
...
void SineAsPhased::nextBuffer(Buffer & outputBuffer, unsigned outBufNum) noexcept(false) {
	Sample * buffer = outputBuffer.buffer(outBufNum);		// get pointer to the selected output channel
...
void SineAsScaled::nextBuffer(Buffer & outBuf, unsigned outBufNum) noexcept(false) {
	Sample * buffer = outputBuffer.buffer(outBufNum);		// get pointer to the selected output channel
	float rateRecip = CSL_TWOPI / mFrameRate;					// Calculate the phase increment multiplier
...
void SineAsScaled::nextBuffer(Buffer & outputBuffer, unsigned outBufNum) noexcept(false) {
	Sample * buffer = outputBuffer.buffer(outBufNum);	// get pointer to the selected output channel
