...
void WavetableOscillator::nextBuffer(Buffer & outputBuffer, unsigned outBufNum) noexcept(false) {
...
void CompOrCacheOscillator::nextBuffer(Buffer & outputBuffer, unsigned outBufNum) noexcept(false) {
...
void Sine::nextBuffer(Buffer & outputBuffer, unsigned outBufNum) noexcept(false) {
	SampleBuffer buffer = outputBuffer.buffer(outBufNum);		// get pointer to the selected output channel
...
//	fprintf(stderr, " :%d: %5.3f", numFrames, scaleValue);
	...
	while (phase >= CSL_TWOPI)								// wraparound after 2pi radians
		phase -= CSL_TWOPI;
//	logMsg("Sine nextBuffer: (%d) %6.4f - %6.4f", numFrames, mPhase, phase);
	mPhase = phase;
}
...
void FSine::nextBuffer(Buffer & outputBuffer, unsigned outBufNum) noexcept(false) {
	SampleBuffer buffer = outputBuffer.buffer(outBufNum);	// get pointer to the selected output channel
	float rateRecip = CSL_TWOPI / mFrameRate;
	unsigned numFrames = outputBuffer.mNumFrames;			// the number of frames to fill
	float phase = mPhase;
...
void Sawtooth::nextBuffer(Buffer & outputBuffer, unsigned outBufNum) noexcept(false) {
	SampleBuffer buffer = outputBuffer.buffer(outBufNum);	// get pointer to the selected output channel
...
//	logMsg("Sawtooth nextBuffer: %5.3f - %5.3f", mPhase, phase);
	mPhase = phase;
}
...
void Square::nextBuffer(Buffer & outputBuffer, unsigned outBufNum) noexcept(false) {
	SampleBuffer buffer = outputBuffer.buffer(outBufNum);	// get pointer to the selected output channel
	float rateRecip = 1.0f / mFrameRate;
	unsigned numFrames = outputBuffer.mNumFrames;			// the number of frames to fill
	float phase = mPhase;
...
void Impulse::nextBuffer(Buffer & outputBuffer, unsigned outBufNum) noexcept(false) {
	SampleBuffer buffer = outputBuffer.buffer(outBufNum);		// get pointer to the selected output channel
	unsigned numFrames = outputBuffer.mNumFrames;			// the number of frames to fill
	unsigned i, j;
	int count = mCounter;
