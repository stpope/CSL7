...
void testSweep() {
	Osc vox;								    // wave-table oscillator, defaults to sine
...
// OR
//	vox.setFrequency(110.0f);				    // set const freq
//	vox.setScale(0.1f);					    // set const ampl

// OR
//	Osc vox(gliss, swell);					// plug them in in the c'tor
	
//	vox.dump();
	gliss.reset();
	swell.reset();
	logMsg("Playing swept/crescendo sin with line segments...");
...
void testSimpleSines() {
	...
	sineOsc.dump();
	MulOp mul(sineOsc, 0.2f);				// scale it quieter
	logMsg("playing simple sin...");
	runTest(mul);							// call the test function that runs this for a few seconds
	logMsg("sin done.");
	sleepMsec(250);

	SineAsPhased sineOsc2(freq, 0.2f);		// create a sine-as-phased oscillator
	sineOsc2.dump();
	MulOp mul2(sineOsc2, 0.2f);				// scale it quieter
	logMsg("playing sin-as-phased...");
	runTest(mul2);							// call the test function that runs this for a few seconds
	logMsg("sin done.");
	sleepMsec(250);
...
void testBasicWaves() {
	float freq = 220.0;						// choose a frequency in Hz
	float ampl = 0.2;						// choose an amplitude scale
...
void testSumOfSines1F() {
	...
	sos1.setFrequency(220);
	sos1.createCache();			// make the cached wavetable
	sos1.setScale(0.5);			// make it quiet
...
//    for (int i = 0; i < 200; i += 2)				// dump soe samples
...
void test_SHARC2() {
	...
	mix.addInput(sos2);
	mix.addInput(sos3);
	csl::ADSR adsr(dur, 0.1, 0.04, 0.7, 0.5);		// apply an ADSR to it
	MulOp summ(mix, adsr);
	Freeverb revb(summ);							// mono reverb
