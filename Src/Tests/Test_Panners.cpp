...
void testBigPanMix() {
	...
	float scale = 2.0f / (float) num;			// ampl scale
	Mixer mix(2);							// stereo mixer
...
/// Spatializer with Ambisonics -- this crashes when optimized  :-(
...
void test_Ambi_horiz() {
	...
	for (float i = 0.0f; i < 50.0f; i++) {
		source.setPositionP(i * 24.0f, 0.0f, 3.0f + (i * 0.5f));	// rotate in small steps
		source.dump();
		sndfile.trigger();
		sleepSec(0.25);
	}
...
void test_convolution() {	// Make dual-mono soundfile chain
    ...
    MulOp smul1(*sndfile1, 50.0);
    MulOp smul2(*sndfile2, 50.0);
...
float * load_filter_data(char * fname, unsigned & siz) {
	...
	if (strcmp(pos, "ir") == 0) {				// raw data with text header
		...
		SampleBuffer bfr;
		SAFE_MALLOC(bfr, csl::Sample, len);
