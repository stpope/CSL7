...
void testADSR2() {
	...
	float attack = 0.05;			// att time
	float decay = 0.03;			// dec time
	float sustain = 0.1;			// sust value
	float release = 1.5;			// release time
	csl::ADSR adsr(duration, attack, decay, sustain, release);		// constructor
	Osc vox(220);
	vox.setScale(adsr);
	...
	csl::ADSR adsr2(duration, 0.5, 0, 1, 0.5);
	vox.setScale(adsr2);
...
void testADSR_FM() {
	csl::ADSR a_env(3, 0.1, 0.1, 0.5, 1);				// set up 2 standard ADSRs (3-sec duration)
	csl::ADSR i_env(3, 1.5, 0, 1, 0.4);
...
void testEnvScale() {
	...
	csl::ADSR a_env(2, 0.6, 0.1, 0.3, 1);				// amplitude env = std ADSR
...
void testFancy_FM() {
	...
	csl::ADSR a_env(3, 0.04, 0.2, 0.5, 1);			// set up 2 standard ADSRs (3-sec duration)
	csl::ADSR i_env(3, 1.0, 0, 1, 1.0);
	
	Envelope mVibEnv(kExpon, 3, 0.0, 0.1, 2, 1.0, 3, 0.1);	// vibrato envelope (expon triangle)
	csl::ADSR mChiffEnv(3, 0.01, 0.01, 0.0, 2);		// attack-chiff envelope
	...
	logMsg("playing Fancy FM w. attack chiff & freq/ampl envs\n");
...
UnitGenerator * sosNote(float dur, float frq, float amp) {
	...
	for (int i = 0; i < 10; i++)					// add 30 partials per note
...
//	"AR sine 2",				testARSin2,				"",
	...
	"Envelope scaling (clicks)",	testEnvScale,		"Test using an envelope as VCA (clicks)",
