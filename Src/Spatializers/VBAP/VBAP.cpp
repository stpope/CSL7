...
void SpeakerSetLayout::findSpeakerPairs() noexcept(false) {
...
void SpeakerSetLayout::findSpeakerTriplets() noexcept(false) {
	unsigned numSpeakers = mSpeakerLayout->numSpeakers();
...
void VBAP::nextBuffer(Buffer &outputBuffer) noexcept(false) {
	this->nextBuffer(outputBuffer, 0);
}

void VBAP::nextBuffer(Buffer &outputBuffer, unsigned outBufNum) noexcept(false) {
