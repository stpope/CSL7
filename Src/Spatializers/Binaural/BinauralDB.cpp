...
void HRTFDatabase::loadFromFolder (const char *folder) noexcept(false) {
...
void HRTFDatabase::loadFromFile (const char *listname) noexcept(false) {
...
void HRTFDatabase::loadFromDB (const char *dbName) noexcept(false) {
	char lineBuffer[CSL_NAME_LEN];
	char tok[8];
	char nam[16];
...
void HRTFDatabase::storeToDB(const char *filename, const char *sname) noexcept(false) {
...
void HRTFDatabase::convertDB(const char *listname) noexcept(false) {
	char lineBuffer[CSL_NAME_LEN];
	char filename[CSL_NAME_LEN];
	char tok[8];
	FILE * listFile;									// text file with the list of HRTF files in the directory
