...
void RingBufferTap::nextBuffer(Buffer &outputBuffer) noexcept(false){
...
void RingBufferTap::nextBuffer(Buffer &outputBuffer, unsigned outBufNum) noexcept(false) {
...
void RingBufferTap::destructiveNextBuffer(Buffer &outputBuffer) noexcept(false) {
...
void RingBufferTap::destructiveNextBuffer(Buffer &outputBuffer, unsigned outBufNum) noexcept(false) {
	// get everything into registers
	unsigned numFrames = outputBuffer.mNumFrames;
	unsigned currentFrame = mCurrentFrame;
	unsigned ringBufferStartFrame = mLoopStartFrame;	
...
unsigned RingBufferTap::seekTo(int position, SeekPosition whence) noexcept(false){
	switch(whence) {
		case kPositionStart:
...
void RingBuffer::nextBuffer(Buffer &outputBuffer) noexcept(false) {
...
void RingBuffer::destructiveNextBuffer(Buffer &outputBuffer) noexcept(false) {
	return mTap.destructiveNextBuffer(outputBuffer);
}

void RingBuffer::writeBuffer(Buffer &inputBuffer) noexcept(false) {
	Writeable::writeBuffer(inputBuffer);
	mCurrentWriteFrame = mTempCurrentWriteFrame;
	return;
}

void RingBuffer::sumIntoBuffer(Buffer &inputBuffer) noexcept(false) {
...
unsigned RingBuffer::seekTo(int position) noexcept(false) {
...
void RingBuffer::writeBuffer(Buffer &inputBuffer, unsigned bufNum) noexcept(false) {
...
void RingBuffer::sumIntoBuffer(Buffer &inputBuffer, unsigned bufNum) noexcept(false) {
	// get everything into registers
...
unsigned BufferStream::seekTo(int position, SeekPosition whence) noexcept(false) {
	switch(whence) {
		case kPositionStart:
...
void BufferStream::nextBuffer(Buffer & outputBuffer) noexcept(false) {
	UnitGenerator::nextBuffer(outputBuffer);
	mCurrentFrame = mTempCurrentFrame;	// remember state
	return;
}

void BufferStream::writeBuffer(Buffer &inputBuffer) noexcept(false) {
	Writeable::writeBuffer(inputBuffer);
	mCurrentFrame = mTempCurrentWriteFrame; // remember state
	return;
}

void BufferStream::nextBuffer(Buffer &outputBuffer, unsigned outBufNum) noexcept(false) {
										// get everything into registers
	unsigned numFrames = outputBuffer.mNumFrames;
	unsigned currentFrame = mCurrentFrame;
...
void BufferStream::writeBuffer(Buffer &inputBuffer, unsigned bufNum) noexcept(false) {
						// get everything into registers
	unsigned numFrames = inputBuffer.mNumFrames;
	unsigned currentFrame = mCurrentFrame;
	unsigned bufferEndFrame = mBuffer->mNumFrames;
