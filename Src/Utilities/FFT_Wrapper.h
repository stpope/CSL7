...
#define CSL_FFT_WRAPPER_H
				...
				// I now use compiler flags for these, so different apps can share the same code base.
...
public:							/// Constuctor sets up twiddle factor tables
	...
	virtual void nextBuffer(Buffer & in, Buffer & out) noexcept(false) = 0;
...
public:
	...
	~FFTW_Wrapper();
								/// run the transform between in and out
	void nextBuffer(Buffer & in, Buffer & out) noexcept(false);
