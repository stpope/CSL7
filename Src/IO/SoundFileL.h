...
//	LSoundFile(char * buffer);						///< virtual version uses a sample buffer
	...
	virtual void openForRead(bool load = true) noexcept(false);	///< open file and get stats
	...
	void readBufferFromFile(unsigned numFrames);
								...
								/// UGen operations
	void nextBuffer(Buffer &outB) noexcept(false);				///< copy next buffer from cache
	void writeBuffer(Buffer &inputBuffer) noexcept(false);		///< write a buffer of data into the file
	void writeBuffer(Buffer &inB, unsigned fromFrame, unsigned toFrame) noexcept(false);
