...
bool Abst_SoundFile::readTags() noexcept(false) {
#ifdef USE_TAGS
	if ( ! mProperties) 
...
void Abst_SoundFile::nextBuffer(Buffer &outputBuffer) noexcept(false) {
...
void SoundCue::nextBuffer(Buffer &outputBuffer) noexcept(false) {
	unsigned numFrames = outputBuffer.mNumFrames;
