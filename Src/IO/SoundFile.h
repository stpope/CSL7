...
public:									/// Constructor. Values not passed default to null.
	...
	virtual void nextBuffer(Buffer &outB) noexcept(false);
													/// write a buffer of data into the file
	virtual void writeBuffer(Buffer &inB) noexcept(false) = 0;
...
protected:
	...
	bool readTags() noexcept(false);				///< read the ID3 or other tags. Returns true if able to read them.
};
...
//	SoundCue(string name, int start = 1, int stop = -1);
	...
	void dump(void);					///< pretty-print me
						/// UGen operations
	void nextBuffer(Buffer & outputBuffer) noexcept(false);	///< copy next buffer from cache
