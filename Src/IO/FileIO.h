...
public:
	...
	void start(float seconds = 0) noexcept(false);
	void stop() noexcept(false);
	void close() noexcept(false);
	void test() noexcept(false) { };	///< test the IO's graph
										///< Get the current input from the sound card
	Buffer & getInput() noexcept(false) { return mInBuffer; };
	Buffer & getInput(unsigned numFrames, unsigned numChannels) noexcept(false) { return mInBuffer; };
