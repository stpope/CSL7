...
void JSoundFile::openForRead(bool load) noexcept(false) {
...
//void JSoundFile::openForRead(juce::MemoryInputStream * memStrm) noexcept(false) {
//    mMode = kSoundFileRead;
...
unsigned JSoundFile::seekTo(int position, SeekPosition whence) noexcept(false) {
	int whenceInt;
	if (this->isCached()) {
...
void JSoundFile::writeBuffer(Buffer &inputBuffer) noexcept(false) {
...
void JSoundFile::writeBuffer(Buffer &inputBuffer, unsigned fromFrame, unsigned toFrame) noexcept(false) {
	unsigned numFrames = inputBuffer.mNumFrames;	
											// JUCE write fcn
