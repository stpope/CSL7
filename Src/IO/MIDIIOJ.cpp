...
int MIDIIO::countInDevices() {
//	juce::StringArray midiDevs = juce::MidiInput::getDevices();
	return (int) mInDevices.size();
}

int MIDIIO::countOutDevices() {
//	juce::StringArray midiDevs = juce::MidiOutput::getDevices();
	return (int) mOutDevices.size();
}
...
// Print the device table to std out
/*
 Found 4 MIDI ins and 5 MIDI outs
	 MIDI in devices
		 0 = MIDIPLUS TBOX 2x2 Midi In 1
		 1 = Faders
		 2 = IAC Driver Bus 1
		 3 = Nord Stage 3 MIDI Output
	 Def: 0

	 MIDI out devices
		 0 = Nord Stage 3
		 1 = MIDIPLUS TBOX 2x2 Midi Out 2
		 2 = IAC Driver Bus 1
		 3 = Nord Stage 3 MIDI Input
		 4 = Kontakt 7 Virtual Input
	 Def: 0
